...
public class AdjustEvnColor : MonoBehaviour {
    ...
    {
        return screen.GetComponent<VideoPlayer>().texture;
        //return screen.material.mainTexture;
        //return imx.PlayingParams.mCurrentTexture;
    }
    ...
    void SetColor()
    {
        Color color = new Color(averageColor[0], averageColor[1], averageColor[2]);
        Renderer[] rds = GetComponentsInChildren<Renderer>();
        foreach (Renderer rd in rds)
        {
            rd.material.SetColor("_ScreenColor", color);
        }
    }
